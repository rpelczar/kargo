...
func (w *webhook) Default(ctx context.Context, obj runtime.Object) error {
	...
	if req.Operation == admissionv1.Create || req.Operation == admissionv1.Update {
		if verReq, ok := helpers.ReverifyAnnotationValue(stage.Annotations); ok {
			var oldVerReq *helpers.VerificationRequest
			if oldStage != nil {
				oldVerReq, _ = helpers.ReverifyAnnotationValue(oldStage.Annotations)
			}
			...
			if oldStage == nil || oldVerReq == nil || !verReq.Equals(oldVerReq) {
				...
				if !verReq.ControlPlane {
					...
					verReq.Actor = helpers.FormatEventKubernetesUserActor(req.UserInfo)
				}
				stage.Annotations[kargoapi.AnnotationKeyReverify] = verReq.String()
			}
		}

		if verReq, ok := helpers.AbortVerificationAnnotationValue(stage.Annotations); ok {
			var oldVerReq *helpers.VerificationRequest
			if oldStage != nil {
				oldVerReq, _ = helpers.AbortVerificationAnnotationValue(oldStage.Annotations)
			}
