...
func ListAvailableFreight(
	...
	for _, req := range s.Spec.RequestedFreight {
		...
		freightFromWarehouse, err := ListFreight(ctx, warehouse, c, listOpts)
		if err != nil {
			return nil, err
		}
...
// Stage.
func IsFreightAvailable(s *kargoapi.Stage, freight *kargoapi.Freight) bool {
	if s == nil || freight == nil || s.Namespace != freight.Namespace {
		return false
	}
	if IsApprovedFor(freight, s.Name) {
		return true
	}
	for _, req := range s.Spec.RequestedFreight {
		if freight.Origin.Equals(&req.Origin) {
			...
			for _, source := range req.Sources.Stages {
				if IsVerifiedIn(freight, source) {
					return req.Sources.RequiredSoakTime == nil ||
						GetLongestSoak(freight, source) >= req.Sources.RequiredSoakTime.Duration
				}
			}
		}
	}
