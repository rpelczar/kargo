...
type Promoter interface {
	...
	// using the provided dirsdk.PromotionStepContext. Implementations may
	// indirectly modify that context through the returned
	// dirsdk.PromotionStepResult to allow subsequent promotion steps to access
	// the results of its execution.
	Promote(context.Context, *dirsdk.PromotionStepContext) (*dirsdk.PromotionStepResult, error)
}

// RetryablePromoter is an interface for components that implement the logic for
// execution of the individual PromotionSteps of a user-defined promotion
// process that may be retried in the event of failure.
...
// to the environment of the PromotionStep.
func StepEnvWithOutputs(outputs dirsdk.State) PromotionStepEnvOption {
	return func(env map[string]any) {
		env["outputs"] = outputs
	}
}
...
func StepEnvWithTaskOutputs(alias string, outputs dirsdk.State) PromotionStepEnvOption {
	return func(env map[string]any) {
...
// Promotion is marked as failed. If the Promoter is also a RetryablePromoter,
// its timeout is used as the default. Otherwise, the default is 0 (no limit).
func (s *PromotionStep) GetTimeout(promoter Promoter) *time.Duration {
...
// abandoned and the entire Promotion is marked as failed. If the Promoter is
// also a RetryablePromoter, its threshold is used as the default. Otherwise,
// the default is 1.
func (s *PromotionStep) GetErrorThreshold(promoter Promoter) uint32 {
	fallback := uint32(1)
	if retryCfg, isRetryable := promoter.(RetryablePromoter); isRetryable {
...
type PromotionResult struct {
	...
	HealthCheckSteps []dirsdk.HealthCheckStep
...
func getTaskOutputs(alias string, state dirsdk.State) dirsdk.State {
	if namespace := getAliasNamespace(alias); namespace != "" {
		taskOutputs := make(dirsdk.State)
