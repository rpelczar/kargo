...
func init() {
	Register(newHelmTemplater())
}

func (h *helmTemplater) pathIsFile(path string) bool {
	ext := filepath.Ext(path)
	return ext == ".yaml" || ext == ".yml"
}

// helmTemplater is an implementation of the Promoter interface that renders a
// Helm chart.
type helmTemplater struct {
	schemaLoader gojsonschema.JSONLoader
}

// newHelmTemplater returns an initialized helmTemplater.
func newHelmTemplater() *helmTemplater {
	r := &helmTemplater{}
...
func (h *helmTemplater) Promote(
	...
	cfg, err := ConfigToStruct[builtins.HelmTemplateConfig](stepCtx.Config)
	if err != nil {
		return failure, fmt.Errorf("could not convert config into %s config: %w", h.Name(), err)
	}

	return h.template(ctx, stepCtx, cfg)
}

func (h *helmTemplater) template(
	ctx context.Context,
	stepCtx *dirsdk.PromotionStepContext,
	cfg builtins.HelmTemplateConfig,
) (*dirsdk.PromotionStepResult, error) {
...
func (h *helmTemplater) composeValues(workDir string, valuesFiles []string) (map[string]any, error) {
...
func (h *helmTemplater) newInstallAction(
	cfg builtins.HelmTemplateConfig,
...
func (h *helmTemplater) loadChart(workDir, relPath string) (*chart.Chart, error) {
...
func (h *helmTemplater) checkDependencies(chartRequested *chart.Chart) error {
...
func (h *helmTemplater) writeOutput(cfg builtins.HelmTemplateConfig, rls *release.Release, outPath string) error {
	var (
		manifests     bytes.Buffer
		outPathIsFile = h.pathIsFile(cfg.OutPath)
	)
