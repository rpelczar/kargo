...
func TestFakeEngine_CheckHealth(t *testing.T) {
	t.Run("without function injection", func(t *testing.T) {
		engine := &FakeEngine{}
		res := engine.CheckHealth(context.Background(), dirsdk.HealthCheckContext{}, nil)
	...
	t.Run("with function injection", func(t *testing.T) {
		...
		steps := []dirsdk.HealthCheckStep{{Kind: "mock"}}

		engine := &FakeEngine{
			CheckHealthFn: func(
				givenCtx context.Context,
				givenHealthCtx dirsdk.HealthCheckContext,
				givenSteps []dirsdk.HealthCheckStep,
			) kargoapi.Health {
				assert.Equal(t, ctx, givenCtx)
